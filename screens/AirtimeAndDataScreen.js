...
>
      ...
      <View sx={{ flexDirection: 'row', mb: 20, justifyContent: 'center' }}>
        <Pressable onPress={() => setMode('airtime')} style={{ backgroundColor: mode === 'airtime' ? 'green' : '#e5e7eb', padding: 10, borderRadius: 6, marginRight: 10 }}>
          <Text style={{ color: mode === 'airtime' ? 'white' : 'black' }}>Airtime</Text>
        </Pressable>
        <Pressable onPress={() => setMode('data')} style={{ backgroundColor: mode === 'data' ? 'green' : '#e5e7eb', padding: 10, borderRadius: 6 }}>
  ...
  fetchHistory();
      ...
      {loading ? (
        ...
        history.map(item => (
          <View
            key={item.id}
            sx={{ bg: 'green', borderRadius: 8, p: 12, mb: 8 }}
          >
